...
from app.models import Expense, Category, Setting, User
...
async def get_summary(
    ...
    settings_stmt = select(Setting).filter(Setting.user_id == current_user.id)
    settings_res = await db.execute(settings_stmt)
    user_settings = settings_res.scalar_one_or_none()
    monthly_budget = user_settings.monthly_budget if user_settings else Decimal("10000.00")
...
async def get_by_month(
    ...
    for row in rows:
        ...
        if 0 <= month_idx < 12:
            monthly_map[month_names[month_idx]] = row.amount
